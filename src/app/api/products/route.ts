...
export async function GET(req: NextRequest) {
    ...
    try {
        let query = `
            SELECT pm.id AS ProductMasterId, pm.name, pm.description, pd.price, c.name as category, s.name as sizename, 
        ...
        if (search) {
            query += ` AND (LOWER(pm.name) LIKE ? OR LOWER(c.name) LIKE ? OR LOWER(b.name) LIKE ?)`;
